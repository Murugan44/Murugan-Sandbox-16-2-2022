({
    ...
    handlePSClick : function(component, event, helper) {	
        ...
        var url='/sfdcpage/'+encodeURIComponent('/apex/VG_SAPI_Step1?id=')+recordId+encodeURIComponent('&Currency=')+CC+encodeURIComponent('&portalname=SAPI');
    ...
    handlePTClick: function(component, event, helper){
        ...
        var CC=component.get('v.CurCode');
         var url='/sfdcpage/'+encodeURIComponent('/apex/VG_PolicyPaymentTax?id=')+recordId+encodeURIComponent('&Currency=')+CC;
        var urlEvent = $A.get("e.force:navigateToURL");
        urlEvent.setParams({
            "url": url
        });
    ...
    doInit : function(component, event, helper) {       
        ...
        action.setCallback(this, function(response) {
            var action = component.get("c.getVSR");
            action.setParams({
                "polId": recordId
            });
            action.setCallback(this, function(response) {
                var state=response.getState();
                if(state=="SUCCESS"){
                    component.set("v.isVSR", response.getReturnValue());                   
                }
            });
            $A.enqueueAction(action);
            var state=response.getState();
            if(state=="SUCCESS"){
                ...
                if((pol.Policy_Status__c == 'Bound' || pol.Policy_Status__c=='Booked')){//} && pol.VG_Sub_Status__c=='Approved'){                                                  
